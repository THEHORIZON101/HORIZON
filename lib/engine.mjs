...
export function analyzeReverie() {
  const publishedEligibility = {
    minimumAge: 13,
    maximumAge: 27,
    studentRequired: true,
    minimumTeamSize: 1,
    maximumTeamSize: 3,
    companiesExcluded: true,
  };
  const integrityWitness = {
    age: 26,
    student: true,
    teamSize: 1,
    company: false,
  ...
  const publishedChecks = [
    integrityWitness.age >= publishedEligibility.minimumAge && integrityWitness.age <= publishedEligibility.maximumAge,
    integrityWitness.student === publishedEligibility.studentRequired,
    integrityWitness.teamSize >= publishedEligibility.minimumTeamSize && integrityWitness.teamSize <= publishedEligibility.maximumTeamSize,
    integrityWitness.company === !publishedEligibility.companiesExcluded,
  ];
  const passesPublishedRules = publishedChecks.every(Boolean);
  const safeguardsPresent = {
    projectMustBeginDuringEvent: false,
    priorWorkMustBeDisclosed: false,
  };
  ...
  return {
    title: "A mature project built years earlier can pass every published rule.",
    summary: "The event is presented as a high-school hackathon, but the published entry rules allow students ages 13–27 and do not say when work must begin or require prior work to be disclosed. Archemidy found one exact entrant the written rules do not reject.",
    findings: [
      {
        id: "integrity-gap",
        type: "Uncovered case",
        severity: "critical",
        title: "A three-year-old project can enter beside a new student project",
        summary: "A 26-year-old student entering alone with a mature project satisfies every eligibility check that is actually published.",
        clauseA: "Website · “largest virtual high school hackathon”",
        clauseB: "Devpost rules · students ages 13–27; teams of 1–3",
        witness: [
          ["Entrant", "26-year-old graduate student"],
          ["Team", "1 person; not a company"],
          ["Project", "Mature software built in 2023"],
          ["Deliverables", "Repository, demo, documentation"],
          ["Published result", passesPublishedRules ? "No rule rejects this entry" : "Entry rejected"],
        ],
        proof: `${publishedChecks.filter(Boolean).length}/${publishedChecks.length} published eligibility checks pass; ${Object.values(safeguardsPresent).filter(Boolean).length}/2 prior-work safeguards are present`,
        fix: "Define who the event is for, then require projects to begin during the hacking period—or require prior-work disclosure and judge only the work created during the event.",
      },
      {
        id: "deadline",
        type: "Problem proven",
